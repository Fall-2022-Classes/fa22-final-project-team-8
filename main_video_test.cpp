...
void etch_a_sketch(SpriteCore *cursor_p, Ps2Core *ps2_p, XadcCore *adc_p,
	...
	cursor_p->bypass(0); //enable cursor
	cursor_p->wr_ctrl(0x1B); //initiate cursor

	x = 320; 				//set cursor to middle of screen
	y = 200; 				//
	cursor_p->move_xy(x, y); //
	...
	while (1) {
		...
		if(sw0) cursor_p->wr_ctrl(3 << 3 | 2); //display eraser frame
		else cursor_p->wr_ctrl(3 << 3 | (1 + 2*size) ); //display brush frame of appropriate size
		...
		cursor_p->move_xy(x, y); //move cursor
		...
		cursor_p->wr_color(color12bit); //set cursor color
...
SpriteCore cursor(get_sprite_addr(BRIDGE_BASE, V3_CURSOR), 1024);
...
int main() {
   while (1) {
	  ...
	  etch_a_sketch(&cursor, &ps2, &adc, &osd, &frame, &sw); //functionality
